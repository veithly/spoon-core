...
class MCPServerManager:
    ...
    def _create_transport(self, config: MCPServerConfig):
        ...
        if config.url:
            if config.url.startswith(("ws://", "wss://")) or config.transport == "websocket":
                return WSTransport(config.url)
            elif config.url.startswith(("http://", "https://")):
                if config.transport == "http":
                    # Use StreamableHttpTransport for HTTP
                ...
                raise MCPServerError("unknown", f"Unsupported URL scheme: {config.url}")
    ...
    async def _start_server_internal(self, server: MCPServerInstance) -> None:
        ...
        try:
            ...
            server.client = MCPClient(transport)
