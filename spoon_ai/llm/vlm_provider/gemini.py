...
class GeminiProvider(LLMBase):
    ...
    def __init__(self, config_path: str = "config/config.toml", config_name: str = "chitchat"):
        ...
        # NOTE: We intentionally do NOT keep a long-lived Client instance here.
        # The google-genai SDK maintains internal async resources; creating a client
        # per request via the context manager prevents shutdown warnings like:
        # "Task was destroyed but it is pending!" / "BaseApiClient.aclose was never awaited".
        self.client = None
    ...
    async def chat(
        ...
        **kwargs
        ...
        try:
            ...
            buffer = ""
            is_content = False
            with genai.Client(api_key=self.config.api_key) as client:
